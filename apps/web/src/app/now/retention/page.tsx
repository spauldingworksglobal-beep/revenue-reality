...
export default function NowRetentionPage() {
  const { capitalItems, setCapitalItems } = useNow();
  ...
  return (
    <WizardShell
      eyebrow="NOW · Business Retention"
      title="What does this business currently need to keep or put back into itself?"
      intro={
        <p>
          Before anything is available to the owner, the business pays what operating requires
          and keeps what it needs — a reserve, working capital, inventory, equipment, debt paydown,
          reinvestment. What&rsquo;s left after that is what&rsquo;s actually distributable.
        </p>
      }
    ...
    >
      ...
      {capitalItems.length === 0 && (
        <p className="text-xs text-ink/50">
          If this business doesn&rsquo;t currently retain anything, that&rsquo;s a valid answer too — leave this empty.
        </p>
      )}
      <p className="text-xs text-ink/50">
        A one-time capital need (like a piece of equipment) is never treated as an ongoing operating
        cost — Revenue Reality keeps these separate so your break-even isn&rsquo;t inflated by something
        you only need to pay for once.
      </p>
    </WizardShell>
  );
}
