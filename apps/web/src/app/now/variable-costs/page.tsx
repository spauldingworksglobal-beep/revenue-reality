  ...
  return (
    <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
      ...
      <p className="mb-2 text-xs text-ink/60">
        Costs that change with each sale but aren&rsquo;t part of Cost of Delivery — these feed{" "}
        <strong>Contribution Margin</strong>, calculated separately from Gross Margin.
      </p>
...
export default function NowVariableCostsPage() {
  const { revenueStreams } = useLifeReality();
  const activeStreams = revenueStreams.filter((s) => s.active);

  return (
    <WizardShell
      eyebrow="NOW · Other Variable Costs"
      title="Are there other costs that change with every sale?"
      intro={<p>Optional — leave this empty if none apply. Nothing here changes your Cost of Delivery.</p>}
      backHref="/now/cogs"
      nextHref="/now/opex"
    >
      {activeStreams.map((s) => (
