...
function StreamCogsEditor({ streamId, streamLabel }: { streamId: string; streamLabel: string }) {
  ...
  function commit(next: CogsInput) {
    try {
      resolveCogsPerUnit(next); // validates (e.g. sellable units > 0) before we save it
  ...
  return (
    <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
      ...
      <p className="mb-2 text-xs text-ink/60">
        What is your <strong>Cost of Goods Sold (COGS)</strong> — or Cost of Delivery — for this?
        This is what it costs you to make or deliver what you sell. Enter it the way you know it.
      </p>
      ...
      {cogs?.method === "PER_BATCH" && (
        <div className="mt-3 flex flex-wrap gap-3">
          ...
          <p className="w-full text-xs text-ink/60">Revenue Reality divides this for you — never do the math yourself.</p>
        </div>
      )}
      ...
      {cogs?.method === "COMPONENT_BUILDUP" && (
        <div className="mt-3 flex flex-col gap-2">
          ...
          <p className="text-xs text-ink/60">e.g. ingredients/materials, packaging, direct labor or co-packer, direct fulfillment.</p>
        </div>
      )}
...
export default function NowCogsPage() {
  const { revenueStreams } = useLifeReality();
  const activeStreams = revenueStreams.filter((s) => s.active);

  return (
    <WizardShell
      eyebrow="NOW · Cost of Delivery"
      title="What does it cost you to make or deliver what you sell?"
      backHref="/now/streams"
      nextHref="/now/variable-costs"
    >
      {activeStreams.map((s) => (
