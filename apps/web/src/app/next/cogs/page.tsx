  ...
  return (
    <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
      ...
      <p className="mb-2 text-xs text-ink/60">
        Will this change in NEXT? Pre-filled from NOW — adjust if delivering this at the new volume
        costs differently (e.g. a co-packer discount, a new supplier, higher-volume packaging).
      </p>
...
export default function NextCogsPage() {
  const { revenueStreams } = useLifeReality();
  const activeStreams = revenueStreams.filter((s) => s.active);

  return (
    <WizardShell
      eyebrow="NEXT · Cost of Delivery"
      title="What will it cost to make or deliver what you sell in NEXT?"
      backHref="/next/streams"
      nextHref="/next/variable-costs"
    >
      {activeStreams.map((s) => (
