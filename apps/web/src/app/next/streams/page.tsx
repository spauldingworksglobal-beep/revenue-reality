...
export default function NextStreamsPage() {
  const { revenueStreams } = useLifeReality();
  const { getStreamInput, updateStreamInput } = useNext();
  ...
  if (activeStreams.length === 0) {
    return (
      <WizardShell
        ...
        nextHref="/next/cogs"
      >
        <p className="text-sm text-ink/60">You can still continue — the NEXT model will simply have no revenue economics yet.</p>
      </WizardShell>
    );
  }
  ...
  return (
    <WizardShell
      eyebrow="NEXT · Revenue Streams"
      title="What does a typical sale look like in this NEXT step?"
      intro={
        <p>
          Pre-filled from NOW — change whatever needs to move. Discontinuing a stream in NEXT? Clear
          its price. Bringing a new one online? Fill it in here even if it isn&rsquo;t active in NOW.
        </p>
      }
      backHref="/next/time"
      nextHref="/next/cogs"
          ...
          <fieldset key={stream.id} className="rounded-lg border border-ink/15 bg-white p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-ink/70">Expected sales / volume for NEXT (optional)</label>
                <input
                  type="text"
                  inputMode="decimal"
