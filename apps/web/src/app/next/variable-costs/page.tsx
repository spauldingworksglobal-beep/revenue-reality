...
function StreamVariableCosts({ streamId, streamLabel }: { streamId: string; streamLabel: string }) {
  ...
  return (
    <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
      ...
      <p className="mb-2 text-xs text-ink/60">Pre-filled from NOW — adjust anything that changes at NEXT&rsquo;s volume or channel mix.</p>
...
export default function NextVariableCostsPage() {
  const { revenueStreams } = useLifeReality();
  const activeStreams = revenueStreams.filter((s) => s.active);

  return (
    <WizardShell
      eyebrow="NEXT · Other Variable Costs"
      title="Do these costs change at NEXT's volume or channel mix?"
      intro={<p>Optional — leave empty if nothing changes from NOW.</p>}
      backHref="/next/cogs"
      nextHref="/next/opex"
    >
      {activeStreams.map((s) => (
