...
function OwnerCard({ ownerId, label, isPrimaryRespondent }: { ownerId: string; label: string; isPrimaryRespondent: boolean }) {
  const { nextBusinessHoursWeek } = useNext();
  const { getOwnerInput, updateOwnerInput } = useNext();
    ...
    <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
      ...
      {isPrimaryRespondent ? (
        <p className="mb-2 text-sm text-ink/70">
          Set on the Time Reality screen: about <strong>{nextBusinessHoursWeek.value} hours/week</strong> in NEXT.
        </p>
      ) : (
        <HoursInputRow
          label={`About how many hours a week should the business use from ${label || "this owner"} in NEXT?`}
      ...
      <div className="mt-3 flex flex-col gap-1">
        <label className="text-xs text-ink/70">Targeted labor compensation in NEXT — a real paycheck this owner should receive (optional)</label>
        <input
          type="text"
          inputMode="decimal"
      ...
      <div className="mt-3 flex flex-wrap gap-3">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-ink/70">Personal money into the business in NEXT — continues at (0 if it should stop)</label>
          <input
            type="text"
            inputMode="decimal"
            placeholder="$0.00"
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-ink/70">Business bills paid personally in NEXT — continues at (0 if it should stop)</label>
          <input
            type="text"
            inputMode="decimal"
            placeholder="$0.00"
      ...
      <p className="mt-2 text-xs text-ink/50">
        Pre-filled from NOW&rsquo;s figures — set either to $0 to signal it should stop in NEXT.
      </p>
    </fieldset>
  );
}

export default function NextOwnersPage() {
  const { owners } = useLifeReality();

  return (
    <WizardShell
      eyebrow="NEXT · Owners"
      title="What should change about what each owner puts in — and gets paid?"
      intro={<p>NEXT is often where unpaid or underpaid labor starts becoming a real paycheck. Set a target if you have one — leave it blank if you don&rsquo;t yet.</p>}
      backHref="/next/growth"
      nextHref="/next/delegation"
    >
      {owners.map((owner) => (
