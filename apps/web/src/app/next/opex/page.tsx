...
export default function NextOpexPage() {
  const { operatingCosts, setOperatingCosts, opexListIsPartial, setOpexListIsPartial } = useNext();
  ...
  function addCost(label: string) {
    ...
    try {
      ...
      setOperatingCosts((prev) => [
        ...prev,
        { id: newId(), scenarioId: "next", category: label.trim(), amount, cadence, knownOrEstimated: "KNOWN", confidence: "STRONG_ESTIMATE", isPartialList: false },
      ]);
      setCategory("");
      setAmountRaw("");
      setError(null);
  ...
  return (
    <WizardShell
      eyebrow="NEXT · Operating Costs"
      title="Do NEXT's continuing costs change from NOW?"
      intro={<p>Pre-filled from NOW. Adjust any amount that will change, remove one that stops, or add a new continuing cost — new costs required specifically to reach NEXT belong on the next screen.</p>}
      backHref="/next/variable-costs"
      nextHref="/next/growth"
    >
      ...
      <div className="rounded-lg border border-dashed border-ink/25 p-4">
        <p className="mb-2 text-sm font-medium">Add another continuing cost</p>
      ...
      <label className="flex items-center gap-2 rounded-lg border border-ink/15 bg-white p-4 text-sm">
        ...
        This isn&rsquo;t everything — there are other continuing costs I haven&rsquo;t listed yet.
      </label>
    </WizardShell>
  );
}
