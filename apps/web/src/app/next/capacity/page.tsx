...
export default function NextCapacityPage() {
  const { capacity, setCapacity } = useNext();
  const [constraintInput, setConstraintInput] = useState("");

  const current = capacity ?? { scenarioId: "next", demandState: "UNSURE" as DemandState, constraints: [] };
  ...
  return (
    <WizardShell
      eyebrow="NEXT · Capacity"
      title="Can the business actually produce or deliver at this NEXT level?"
      intro={<p>Optional, but worth answering honestly — a model that's mathematically required but practically unreachable is exactly what this is meant to surface.</p>}
      backHref="/next/retention"
      nextHref="/next/result"
    >
      <fieldset className="rounded-lg border border-ink/15 bg-white p-4">
        <legend className="px-1 text-sm font-medium">How achievable does this NEXT level of demand/capacity feel?</legend>
      ...
      <div className="rounded-lg border border-ink/15 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-ink/60">e.g. oven capacity, staff hours, supplier lead time, shelf space.</p>
