  ...
  return (
    <div className="rounded-md border border-ink/15 p-3">
      ...
      <p className="mt-1 text-xs text-ink/60">
        A specific NEXT profit-distribution target for this owner (optional) — leave blank to let the
        distribution rule above determine it from whatever surplus the model produces.
      </p>
...
export default function NextDistributionPage() {
  const { owners } = useLifeReality();
  const { distributionPolicy, setDistributionPolicy, distributionPercents, setDistributionPercent } = useNext();
  ...
  return (
    <WizardShell
      eyebrow="NEXT · Ownership Economics"
      title="How should profit be distributed once this NEXT model is running?"
      intro={
        <p>
          Pre-filled from NOW&rsquo;s rule if one was set — change it if NEXT should distribute
          differently. Never assumed from ownership percentage.
        </p>
      }
      backHref="/next/delegation"
      nextHref="/next/retention"
    >
          ...
          {RULES.map((r) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                onClick={() => setDistributionPolicy({ scenarioId: "next", rule: r.value })}
